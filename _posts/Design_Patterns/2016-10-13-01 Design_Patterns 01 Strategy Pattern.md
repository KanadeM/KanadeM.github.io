...
There is a game - simulation ducks. In this game, player can simulate red duck and green duck. Both kinds of duck can swim and quack. The only difference is the color.

# Design the game from object oriented view

Bob take the responsibilty of this project. From oo perspective, Duck should be a super class. Green duck and red duck will extend the super class. So, the code should be like this:
...
There is a new requirement: <span style="background:yellow"> both red duck and green duck can fly.</span> This example is easy since Bob just need to add a new function to super class:
...
<span style="color:red">One behavior of ducks is various. However, some kinds of ducks have same behavior. Therefore, if Bob implements a behavior in super class, sub classes need to override it.</span> It makes the extension and maintaining of the project difficult. We are aiming at Time complexity $O(N)$ of extension of the project.
...
Flying behavior, quacking behavior and swimming behavior are various. In addition, they can be combined at will. <span style="background: yellow">Therefore, if one kind of behaviors are put into a cluster, the project will be easier. </span>

## What is Strategy Pattern?

<span style="background: yellow">Encapsulate the behavior interface respectively, implement the algorithm cluster, put the behavior interface object in the superclass, and set the behavior object in the subclass. The principle is: separate the changing parts, encapsulate the interface, and program various functions based on the interface. This mode allows behavioral changes to the algorithm to be independent of the user of the algorithm.</span>

## Code the project from stratege pattern view
